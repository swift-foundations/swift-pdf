...
extension PDF {
    ...
    ///
    /// Top-level convenience for batch rendering.
    /// Forwards to `render.renderBatch()`.
    ///
    /// ## Usage
    ///
    /// ```swift
    ...
    public func batch(
        ...
        try await render.renderBatch(htmls, to: directory)
    }

    /// Render multiple HTML strings, collecting all URLs
    ///
    /// Top-level convenience for synchronous batch rendering.
    /// Forwards to `render.renderBatchSync()`.
    ///
    /// ## Usage
    ///
    /// ```swift
    ...
    /// - Throws: Rendering errors
    public func batchSync(
        _ htmls: [String],
        to directory: URL
    ) async throws -> [URL] {
        try await render.renderBatchSync(htmls, to: directory)
    }
}
