...
struct StressTests {
    ...
    func test1MPDFs() async throws {
        ...
        let config = PrintingConfiguration(
            ...
            webViewAcquisitionTimeout: 600, // 10 minute timeout
    ...
    func test100kPDFs() async throws {
        ...
        // Configure for high throughput
        let config = PrintingConfiguration(
            maxConcurrentOperations: 8, // Max concurrent WebViews
            webViewAcquisitionTimeout: 300, // 5 minute timeout
            progressHandler: { completed, total in
                Task {
                    _ = await tracker.recordCompletion()
                }
    ...
    func test10kPDFs() async throws {
        ...
        let documents = (1...count).map { i in
            Document(
                ...
                html: "<html><body><p>Doc \(i)</p></body></html>"
            )
        }
    ...
    func test1kComplexPDFs() async throws {
        ...
        let documents = (1...count).map { i in
            Document(
                fileUrl: output.appendingPathComponent("complex-\(i).pdf"),
                html: complexHTML.replacingOccurrences(of: "{{ID}}", with: "\(i)")
            )
        }
        ...
        let config = PrintingConfiguration(
            maxConcurrentOperations: 6, // Fewer concurrent for complex rendering
            webViewAcquisitionTimeout: 120
        )
